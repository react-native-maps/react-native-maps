...
export default class ExpoMarker extends React.Component {
  ...
  showCallout() {
    logger.warn('showCallout() is unsupported');
  }
  // Hides the callout for this marker
  hideCallout() {
    logger.warn('hideCallout() is unsupported');
  }
  ...
  render() {
    ...
    const markerProps = {
      ...
      onDragStart(nativeEvent) {
        if (props.onDragStart) props.onDragStart({ nativeEvent });
      },
      onDrag(nativeEvent) {
        if (props.onDrag) props.onDrag({ nativeEvent });
      },
      onDragEnd(nativeEvent) {
        if (props.onDragEnd) props.onDragEnd({ nativeEvent });
      },
      opacity,
      draggable,
      title: markerTitle,
    ...
    if (isCustom) {
      return <ExpoDivIcon {...markerProps} />;
    } else if (pinColor) {
      return (
        <ExpoDivIcon {...markerProps}>
          <View
            style={{
...
ExpoMarker.defaultProps = {
  ...
  calloutAnchor: /** Leaflet: `popupAnchor` */ [0.5, 1],
  calloutOffset: /** Unused: use `calloutAnchor` instead */ [0.5, 1],
...
class ReactDivIcon extends MapLayer {
  ...
  getPopupRelativePointFromRect(point, element) {
    ...
    const { x, y } = ofPoint(point);
    const { width, height } = element;
    return { x: x * width - width / 2, y: -y * height };
  }
  ...
  updateLeafletElement(fromProps, toProps) {
    ...
    if (toAnchor !== fromAnchor) {
      ...
      const iconAnchor = this.getRelativePointFromRect(
        toAnchor,
        this.leafletElement.getElement().getBoundingClientRect()
      );
      // Update the option for internal usage
    ...
    if (toCalloutAnchor !== fromCalloutAnchor) {
      ...
      // this.tryUpdatePopupAnchor(toCalloutAnchor);
