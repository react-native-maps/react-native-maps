...
import {
  featureGroup as FeatureGroup,
  point as createPoint,
  latLngBounds as boundsForCooords,
} from 'leaflet';
...
const logUnsupportedProps = (...props) => {
  for (const prop of props) {
    logger.warn(`Prop \`${prop}\` is not supported`);
  }
};
export default class extends React.Component {
  ...
  setNativeProps(props) {
    if ('initialRegion' in props) {
      if (this._initialRegion) {
        this._initialRegion = props.initialRegion;
      } else if (props.initialRegion) {
        this.leafletElement.fitBounds(convertRegion(props.initialRegion));
      }
    } else {
      logUnsupportedProps(
  ...
  getMapBoundaries = () => {
    const northEast = this.leafletElement.getNorthEast();
    const southWest = this.leafletElement.getSouthWest();
    return {
      northEast: {
...
function transformPropsFromCamera({ camera, region }) {
  if (camera) {
    ...
    if ('center' in camera) transformProps.center = camera.center;
    ...
    if ('altitude' in camera) transformProps.center = camera.altitude;
