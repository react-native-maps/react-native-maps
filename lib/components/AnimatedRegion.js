...
const configTypes = ['latitude', 'longitude', 'latitudeDelta', 'longitudeDelta'];
...
export default class AnimatedMapRegion extends AnimatedWithChildren {
  constructor(valueIn = {}) {
    super();
    this.latitude = this.setInitialValue(valueIn.latitude, defaultValues.latitude);
    this.longitude = this.setInitialValue(valueIn.longitude, defaultValues.longitude);
    this.latitudeDelta = this.setInitialValue(valueIn.latitudeDelta, defaultValues.latitudeDelta);
    this.longitudeDelta = this.setInitialValue(valueIn.longitudeDelta, defaultValues.longitudeDelta);
    this._regionListeners = {};
  }

  setInitialValue(value, fallback) {
    return value instanceof Animated.Value ? value : new Animated.Value(fallback);
  }
  ...
  spring(config) {
    ...
    for (const type of configTypes) {
      if (config.hasOwnProperty(type)) {
        animations.push(Animated.spring(this[type], {
          ...config,
          toValue: config[type]
        }));
      }
  ...
  timing(config) {
    ...
    for (const type of configTypes) {
      if (config.hasOwnProperty(type)) {
        animations.push(Animated.timing(this[type], {
          ...config,
          toValue: config[type],
