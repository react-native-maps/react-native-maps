...
class MapView extends React.Component {
  ...
  zoomIn(duration) {
    this._runCommand('zoomIn', [duration || 500]);
  }

  zoomOut(duration) {
    this._runCommand('zoomOut', [duration || 500]);
  }

  zoomTo(zoom, duration) {
    this._runCommand('zoomTo', [zoom, duration || 500]);
  }

  zoomBy(zoom, duration) {
    this._runCommand('zoomBy', [zoom, duration || 500]);
  }
