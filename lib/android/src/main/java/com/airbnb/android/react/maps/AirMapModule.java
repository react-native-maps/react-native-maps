...
public class AirMapModule extends ReactContextBaseJavaModule {
  ...
  public void takeSnapshot(final int tag, final ReadableMap options, final Promise promise) {
    ...
    // Add UI-block so we can get a valid reference to the googleMap-view
    ...
    uiManager.addUIBlock(new UIBlock() {
      public void execute(NativeViewHierarchyManager nvhm) {
        ...
        if (view.googleMap == null) {
          promise.reject("AirMapView.googleMap is not valid");
          return;
        }
        view.googleMap.snapshot(new GoogleMap.SnapshotReadyCallback() {
