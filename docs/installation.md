...
1. If you need `GoogleMaps` support also
    ...
    - In `AppDelegate.m`, add `#import <GoogleMaps/GoogleMaps.h>` before `@implementation AppDelegate`. In `- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions`, add `[GMSServices provideAPIKey:@"YOUR_GOOGLE_MAP_API_KEY"];`
