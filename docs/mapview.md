...
| `userLocationPriority` | 'balanced'\|'high'\|'low'\|'passive' | 'high' | Set power priority of user location tracking. **Note:** Android only.
| `userLocationUpdateInterval` | `Number` | 5000 | Interval of user location updates in milliseconds. **Note:** Android only.
