...
| `animateCamera`             | `camera: Camera`, `{ duration: Number }`                                               | Animate the camera to a new view. You can pass a partial camera object here; any property not given will remain unmodified. `duration` is not supported for Apple Maps. |
| `setCamera`                 | `camera: Camera`, `{ duration: Number }`                                               | Like `animateCamera`, but sets the new view instantly, without an animation.                                                                                            |
| `animateToRegion`           | `region: Region`, `duration: Number`                                                   | `duration` is not supported for Apple Maps.                                                                                                                             |
