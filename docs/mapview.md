...
| `addressForCoordinate` | `coordinate: Address` | Converts an address to a map coordinate (`LatLng`). Returns a `Promise<LatLng>` **Note** Not supported on Google Maps for iOS.
| `coordinateFromAddress` | `address: LatLng` | Converts a map coordinate to a address (`Address`). Returns a `Promise<Address>` **Note** Not supported on Google Maps for iOS.
