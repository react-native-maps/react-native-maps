...
| `onIdleCamera` | `{ bounding: { lat_max: Number, lon_max: Number, lat_min: Number, lon_min: Number,}, Region }` | Callback that is called once when the camera is idle at its position, such as when the user is done moving the map.(Only for GoogleMaps)
